...
The library contains APIs for interacting with MMPLD files at different levels of abstractions: the [low-level API](#the-low-level-api) leaves the user most control about handling the file. The `mmpld::file` [abstraction](#the-mmpldfile-class) handles the file I/O and the meta-data management for you.
...
The library also provides functions for [converting particle lists](#converting-particle-lists) to different formats, e.g. to convert global radii to per-particle data. Two different implementations for that are available, one that allows you to convert any type of particle list to a compile-time defined format described by a `mmpld::particle_traits` and one to perform arbitrary conversions at runtime. Furthermore, the convenience function `read_as` allows for converting particles on the fly while reading them from disk.
...
If `#define MMPLD_WITHOUT_POSIX_IO` is added before including the library header, the library does not include the I/O traits for POSIX `int` file handles. Use this define when the library should be used in Unreal Engine. Likewise, if `#define MMPLD_WITHOUT_WIN32_IO` is added before including the library header, the library does not include the I/O traits for native Win32 `HANDLE`s. Use this define when the library should be used in in UWP apps.
...
for (decltype(fileHeader.frames) i = 0; i < fileHeader.frames; ++i) {
    ...
    for (decltype(frameHeader.lists) j = 0; j < frameHeader.lists; ++j) {
        ...
        // particles and read and convert the input in bachtes of 16.
...
### Version 1.15
* Up to this version, converting intensity/grey-scale data to multi-channel colour data would splat the intensity as is to all colour channels. This behaviour is unreasonable, because it would make colours clip unless the intensity was already normalised within [0, 1]. The new conversion routine will normalise the value to the available range of the colour channels. There is no backward compatibility for the previous behaviour, because there is no reasonable application case in duplicating and clipping intensity data.
